...
COLS=[("rot2","rot2 k→k+2","cycles (4 4)","ALL-EVEN → dies","even"),
      ("rot1","rot1 k→k+1","cycles (8)","ALL-EVEN → dies","even"),
      ("reduced0246","reduced rot2 {0,2,4,6}","cycles (4 1 1 1 1)","any-odd → lives","odd"),
      ("reduced024","reduced rot2 {0,2,4}","cycles (3 1 1 1 1 1)","any-odd → lives","odd"),
      ("reduced02","reduced rot2 {0,2}","cycles (2 1 1 1 1 1 1)","any-odd → lives","odd")]
...
for col,(id,l1,l2,l3,kind) in enumerate(COLS):
    gen,phe=load(id)
    ...
    axg.text(0.0,1.02,l3,transform=axg.transAxes,fontsize=8.5,color=c2,family="monospace")
    div=len(set(gen[-1]))
