...
(defn -main [& [registration-path trace-path lean-repo agency-endpoint
                authorization-revision output]]
  (when-not (every? some? [registration-path trace-path lean-repo
                           agency-endpoint authorization-revision output])
    (throw (ex-info
            "usage: write_apm_demonstration_launch_authorization.clj REGISTRATION TRACE LEAN_REPO AGENCY_ENDPOINT AUTHORIZATION_REVISION OUTPUT"
            {})))
