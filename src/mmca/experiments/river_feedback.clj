(ns mmca.experiments.river-feedback
  ...
  feedback with a MATCHED ablation. `run-river` vs
  `run-river-ablated` share the exact Java seed, initial state,
...
(defn divergence
  ...
  [seed width steps]
  (let [rv (:gen (c/run-river seed width steps))
        ab (:gen (c/run-river-ablated seed width steps))]
...
(defn -main [& _]
  (let [seeds (range 1 21) width 80 steps 120
    ...
    (println "run-river vs -ablated | seeds 1-20 | W=80 T=120")
